...
export const Newsletter: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (resp.success) {
        toast.success(`🎉 Registered! Greetings ${firstName || 'Friend'}. Check your logs for connection!`);
        // reset fields
        setFirstName('');
        setLastName('');
        setEmail('');
  ...
  return (
    ...
    >
      <div className="max-w-xl mx-auto bg-white border border-slate-100 rounded-2xl p-6 sm:p-8 shadow-md relative z-10 text-left">
        ...
        {/* Cartoon Face Floating Sticker turned Sleek emerald bubble */}
        <div className="absolute -top-7 left-1/2 transform -translate-x-1/2 bg-emerald-500 text-white rounded-full h-14 w-14 flex items-center justify-center text-xl shadow-md border border-emerald-450 animate-bounce-subtle">
          🍒
        </div>

        <div className="text-center mb-8 mt-4">
          <h2 className="text-xl sm:text-2xl font-bold text-slate-805 uppercase tracking-wide">
            Newsletter Registration
          </h2>
          <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider mt-1.5">
            Stay updated with fresh recipes & custom coupons
          </p>
        </div>

        {/* Form fields styled sleek */}
